...
public class BufferedFileInputStream extends FileInputStream {
  byte[] buf = new byte[8192]; //왜 한 번에 8k만 읽어오냐? 자바는 여러명이 동시에 같이 사용하는 서버프로그램이다.
  //동시에 몇만명이 접속하기 때문에 바이트 배열을 너무 늘리면 서버 메모리가 부족하다.
  ...
  //파일에서 데이터를 읽어 오는것이 시간이 제일 오래 걸리는데 buf는 2번만 읽었기 때문에 시간 단축!
