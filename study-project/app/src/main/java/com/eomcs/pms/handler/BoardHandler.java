...
public class BoardHandler {
  ...
  // => static 옵션을 뺀다.package com.eomcs.pms.handler;

  //Q. 중첩클래스로 사용하는 이유
  //A. 특정 클래스 안에서만 사용할 클래스이기 때문   
  static class Node {
    Board board;
    Node next;
  ...
  Node head;               // Q. 왜 Node 클래스 안에 두지 않는건지?                       
  //A.head 와 tail은 클래스마다 값이 다르기때문
  Node tail;
  ...
  public void add() {
    ...
    if (head == null) {
      ...
      tail = node;              //Q. Node 주소가 같지 않은지?
    }                             //A. 주소를 이동 시키는 것과. 주소를 새 노드에 넣는 것의 차이
  ...
  public void delete() {
    ...
    Board board = findByNo(no);                 //Q. 기존에 indexOf로 했던 이유?
    //A. 리턴값 다름
